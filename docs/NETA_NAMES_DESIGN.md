...
4. Add the verified registry address to `names.js` and ship a wallet signing flow with commit/reveal recovery, fee recheck, transaction receipts and successful register/renew tests. The current UI deliberately keeps registration disabled until this is done.
